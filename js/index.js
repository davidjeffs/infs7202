...
function addMarkers(themap) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            restaurant = $('#restaurant').prop('checked');
                        ...
                        xmlhttpdeals.onreadystatechange = function() {
                            if (this.readyState == 4 && this.status == 200) {
                                ...
                                for (var i = 0; i < deals.length; i++) {
                                    ...
                                    var startdate = data2.start_date
                                    var enddate = data2.end_date
                                    printdeals += "<li class='infoList'><div>" + dealtext + "</div>" +startdate + "<div> to </div>" + enddate + "</li>"
                                }
                                printdeals = '<h3><span style="color: #ffffff;">Current deals:</span></h3><ol>' + printdeals + '</ol>'
                                }
                                else {
                            ...
                            var content = 
                            '<h1 style="text-align: center;"><strong><span style="color: #cf0029;">' + data.name + '</span></strong></h1><h3 style="text-align: center;"><span style="color: #ffffff;"><strong>' + data.address + '</strong></span></h3>' + printdeals + 
'<p style="text-align: center;"><strong>CONTACT</strong></p><p style="text-align: center;">' + data.phone + '</p><p style="text-align: center;">' + data.email + '</p>'
    ...
    xmlhttp.open("GET", "map_db_pull.php?swLat=" + swLat + "&swLng=" + swLng + "&neLat=" + neLat + "&neLng=" + neLng + "&cLat=" + cLat + "&cLng=" + cLng + "&bar=" + bar + "&cafe=" + cafe + "&restaurant=" + restaurant + "&date=" + date, true);
    xmlhttp.send();
